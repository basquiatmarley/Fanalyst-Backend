      ...
      />
        ...
        >
          ...
          <template v-slot:imageUrl="{ row: data }">
            <img  :src="(data.imageUrl != '' ? getApiUrl('.sandbox/'+data.imageUrl) : 'https://placehold.jp/150x150.png')" :class="`w-100px`" alt="image"  />
          </template>
...
const getData = async (params) => {
  try {
    ...
    const response = await ApiService.query("sports-groups", { params : {filter: params }});
